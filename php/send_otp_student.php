...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Query student by username or email
    $stmt = $con->prepare("SELECT id, email FROM tbl_student_users WHERE username = ? OR email = ?");
    if (!$stmt) {
    ...
    if ($result->num_rows === 1) {
        ...
        try {
            ...
            echo "<script>
                ...
                window.location.href = 'verify_otp_student.php';
            </script>";
        } catch (Exception $e) {
