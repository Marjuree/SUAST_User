...
$stmt->bind_param('i', $student_id);  // Assuming 'id' is an integer in tbl_student_users
$stmt->execute();
            ...
            <section class="content">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <button type="button" class="btn btn-success mb-3" data-toggle="modal" data-target="#requestClearanceModal">
                            Request Clearance
                        </button>
                        ...
                        <div id="requestClearance" class="toggle-section" style="display: none;">
                            ...
                            $stmt = $con->prepare($query);
                            $stmt->bind_param('s', $student_id);  // Ensure correct bind_param based on student_id type
                            $stmt->execute();
                            $result = $stmt->get_result();
                            while ($row = $result->fetch_assoc()): 
                                $status = !empty($row['status']) ? htmlspecialchars($row['status']) : "Pending";
                                $badgeClass = ($status == 'Approved') ? 'success' : (($status == 'Rejected') ? 'danger' : 'info');
                            ?>
                        ...
                        <div id="studentUser" class="toggle-section" style="display: block;">
                            ...
                            <div class="form-container">
                                <form class="form-horizontal">
                                    ...
                                    <!-- Add status display for student -->
                                    <div class="form-group row">
